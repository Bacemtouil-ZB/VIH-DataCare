  ...
  return (
    <div className="d-flex align-items-center justify-content-between mb-4">
      <h6 className="mb-0 fw-bold text-dark" style={{ fontSize: "1rem" }}>
        {icon && <i className={`bi ${icon} me-2`} style={{ color: "#2e7d52" }}></i>}
        {title}
      </h6>
      {actionButton ?? (
        !showForm ? (
  ...
  return (
    <div className="mb-4 rounded overflow-hidden border">
      <div className="d-flex justify-content-between align-items-center px-4 py-3"
        style={{ background: "#3e617e", color: "white", cursor: "pointer", userSelect: "none" }}
        onClick={onToggle}>
        <span style={{ fontWeight: 700, fontSize: "0.95rem" }}>
          <i className="bi bi-clock-history me-2"></i>
          {title}
          <span className="ms-2 badge bg-white bg-opacity-25 rounded-pill fw-normal" style={{ fontSize: "0.8rem" }}>{count}</span>
        </span>
...
export function SectionHeader({ title, icon, children }) {
  return (
    <div className="mb-4 rounded overflow-hidden border">
      <div className="d-flex justify-content-between align-items-center px-4 py-3"
        style={{ background: "#3e617e", color: "white", userSelect: "none" }}>
        <span style={{ fontWeight: 700, fontSize: "0.95rem" }}>
  ...
  return (
    <div className="rounded overflow-hidden" style={{ border: `2px solid ${color}`, background: "white" }}>
      <div className="px-4 py-3 d-flex align-items-center gap-2 text-white" style={{ background: color }}>
        ...
        <span style={{ fontWeight: 700, fontSize: "0.95rem" }}>{isModifying ? labelModify : labelCreate}</span>
      </div>
...
export function AutresSignesSection({ title = "Autres signes", appareils, autresSignes, appareilSelectionne, descriptionSigne, onAppareilChange, onDescriptionChange, onAjouter, onSupprimer, onModifierDescription }) {
  ...
  return (
    <div>
      <p className="text-uppercase fw-bold text-secondary mb-3" style={{ fontSize: "0.78rem", letterSpacing: "0.5px" }}>{title}</p>

      {/* ── Ligne d'ajout ── */}
      <div className="d-flex gap-3 flex-wrap align-items-end mb-3">
        <div style={{ flex: "1 1 180px" }}>
          <label className={LABEL_CLS} style={STYLES.thSm}>Appareil</label>
        ...
        <div style={{ flex: "3 1 260px" }}>
          <label className={LABEL_CLS} style={STYLES.thSm}>
      ...
      {/* ── Tableau avec Appareil / Description / Actions séparées ── */}
      {autresSignes.length > 0 && (
        <table className="table table-sm table-hover mb-4">
          <thead className="table-light">
            <tr>
              <th style={{ ...STYLES.thSm, width: "22%" }}>Appareil</th>
              <th style={STYLES.thSm}>Description</th>
              <th style={{ ...STYLES.thSm, width: 90, textAlign: "center" }}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {autresSignes.map((s) => (
              <tr key={s.id}>
                ...
                {/* Description — mode lecture ou édition inline */}
                <td style={{ verticalAlign: "middle" }}>
                  {editingId === s.id ? (
                    ...
                    <span style={{ fontSize: "0.9rem", color: "#1e293b" }}>{s.description}</span>
                  )}
                </td>

                {/* Actions : modifier description + supprimer */}
...
export function BtnModifier({ onClick }) {
  return (
    <button className="btn btn-sm fw-semibold" style={{ border: "1px solid #6ee7b7", color: "#065f46" }} onClick={onClick}>
...
export function Badge({ bg, color, children }) {
  return <span className="badge" style={{ background: bg, color, fontSize: "0.82rem", fontWeight: 600 }}>{children}</span>;
}
  ...
  return (
    <div className="d-flex align-items-center gap-3 mb-4">
      <div className="d-flex align-items-center gap-2 px-3 py-2 rounded"
        style={checked ? STYLES.rasOn : STYLES.rasOff}
        onClick={() => onChange(!checked)}>
        <div style={checked ? STYLES.trackOn : STYLES.trackOff}>
          <div style={checked ? STYLES.thumbOn : STYLES.thumbOff} />
        </div>
        <span style={{ fontWeight: 700, fontSize: "0.9rem", color: checked ? "#166534" : "#475569" }}>
          RAS — Rien à signaler
        </span>
      </div>
      {checked && <small className="text-success"><i className="bi bi-check-circle me-1"></i>Tous les signes sont à Non</small>}
    </div>
  );
}
...
export function SigneCard({ label, value, disabled, onChange }) {
  return (
    <div className="rounded p-2" style={value ? STYLES.cardSigneOn : STYLES.cardSigneOff}>
      <div className="text-uppercase fw-bold text-secondary mb-2" style={{ fontSize: "0.72rem" }}>{label}</div>
      <div className="d-flex gap-1">
        {[true, false].map((val) => (
          <button key={String(val)} className="btn btn-sm flex-fill py-0" disabled={disabled}
            style={{
              borderRadius: 6, border: "none", fontSize: "0.78rem", fontWeight: 600,
              cursor: disabled ? "not-allowed" : "pointer",
              background: value === val ? (val ? "#16a34a" : "#ef4444") : "#f1f5f9",
              color:      value === val ? "white" : "#64748b",
            }}
...
export function ImcField({ imc }) {
  return (
    <div className="form-control form-control-sm d-flex align-items-center gap-2" style={STYLES.imcBox}>
      {imc
        ? <><span style={{ fontWeight: 700, color: imc.color }}>{imc.val}</span><small style={{ color: imc.color }}>{imc.label}</small></>
...
export function BoutonSauvegarder({ saving, isModifying, onClick }) {
  return (
    <button className="btn btn-sm fw-semibold text-white d-flex align-items-center gap-2"
      style={STYLES.btnSave} onClick={onClick} disabled={saving}>
      {saving
...
// Utilisé dans SignesFonctionnels et SignesCliniques pour l'affichage historique
    ...
    <div className="d-flex flex-column gap-1">
      {autresSignes.map((as, i) => (
        <div key={i} className="d-flex align-items-start gap-2">
          ...
          <span style={{ fontSize: "0.875rem", color: "#1e293b", lineHeight: 1.4 }}>{as.description}</span>
        </div>
      ))}
    </div>
  );
