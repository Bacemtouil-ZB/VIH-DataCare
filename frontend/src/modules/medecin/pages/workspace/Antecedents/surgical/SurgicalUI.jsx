...
export default function SurgicalUI({
  ...
  const TABLE_HEADERS = ["Description", "Date d'intervention", "Remarque", "Actions"];

  const renderRow = (item) => (
    <tr key={item.id}>
      <td style={{ fontSize: 13, verticalAlign: "middle" }}>
        {item.description || <span className="text-muted fst-italic">—</span>}
      </td>
  ...
  return (
    <div>
      {/* ── Bouton Ajouter (visible seulement si formulaire fermé) ── */}
      {!showForm && (
      ...
      {/* ── Formulaire ajout / édition (affiché sur demande) ── */}
      {showForm && (
        <FormulaireWrapper
          isModifying={isEditing}
          labelCreate="Ajouter un antécédent chirurgical"
          labelModify="Modifier l'antécédent chirurgical"
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <SectionTitle>Informations de l'intervention</SectionTitle>
            ...
            <div className="d-flex gap-3">
              <div style={{ flex: 1 }}>
                <FieldLabel>Date d'intervention</FieldLabel>
                <input
                  type="date"
      ...
      {/* ── Historique ── */}
      <HistoriqueAccordeon
        title="Historique des interventions chirurgicales"
        count={items.length}
        open={accordeonOpen}
      ...
      >
        <HistoriqueTable
          ...
          items={items}
          renderRow={renderRow}
          emptyMessage="Aucun antécédent chirurgical enregistré."
        />
      </HistoriqueAccordeon>
    </div>
  );
