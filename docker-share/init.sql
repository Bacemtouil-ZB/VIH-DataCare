...
CREATE TABLE ordonnances (
  ...
  statut VARCHAR(50) NOT NULL DEFAULT 'en cours de suivi' 
    CHECK (statut IN ('en cours de suivi', 'perdu de vue', 'en fin de suivi','decedÃƒÆ’Ã‚Â©')),
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);
...
-- ==========================================
-- INSERTION COMPLÃƒÆ’Ã‹â€ TE DES CODES POSTAUX TUNISIENS
-- ==========================================
...
FROM governorates g, (VALUES
  ...
  ('2083', 'KalÃƒÆ’Ã‚Â¢at El Andalous', 36.9583, 10.1250)
) AS data(code, place_name, latitude, longitude)
WHERE g.name = 'Ariana';
...
FROM governorates g, (VALUES
  ...
  ('2014', 'RadÃƒÆ’Ã‚Â¨s', 36.7689, 10.2742),
  ('2015', 'MÃƒÆ’Ã‚Â©grine', 36.7472, 10.2347),
...
SELECT g.id, code, place_name, latitude, longitude
FROM governorates g, (VALUES
  ('9000', 'BÃƒÆ’Ã‚Â©ja', 36.7256, 9.1817),
...
FROM governorates g, (VALUES
  ...
  ('8120', 'AÃƒÆ’Ã‚Â¯n Draham', 36.7833, 8.6833),
...
FROM governorates g, (VALUES
  ...
  ('7160', 'KalÃƒÆ’Ã‚Â¢at Senan', 36.2833, 8.8833),
  ('7170', 'KalÃƒÆ’Ã‚Â¢at Khasba', 36.0667, 8.6833)
) AS data(code, place_name, latitude, longitude)
WHERE g.name = 'Kef';
...
FROM governorates g, (VALUES
  ...
  ('6120', 'GaÃƒÆ’Ã‚Â¢four', 36.3333, 9.3167),
...
FROM governorates g, (VALUES
  ...
  ('3140', 'El AlÃƒÆ’Ã‚Â¢a', 35.6000, 9.4167),
...
FROM governorates g, (VALUES
  ...
  ('1240', 'FÃƒÆ’Ã‚Â©riana', 34.9500, 8.5667),
  ('1250', 'Hassi El Ferid', 34.8667, 8.8167),
  ('1260', 'Mejel Bel AbbÃƒÆ’Ã‚Â¨s', 35.1667, 9.2667),
...
FROM governorates g, (VALUES
  ...
  ('4011', 'Sousse MÃƒÆ’Ã‚Â©dina', 35.8261, 10.6381),
  ('4021', 'Hammam Sousse', 35.8583, 10.6000),
  ('4031', 'Msaken', 35.7333, 10.5833),
  ('4041', 'KalÃƒÆ’Ã‚Â¢a Kebira', 35.9000, 10.5167),
  ('4051', 'KalÃƒÆ’Ã‚Â¢a Seghira', 35.8500, 10.4833),
...
FROM governorates g, (VALUES
  ...
  ('5040', 'TÃƒÆ’Ã‚Â©boulba', 35.6667, 11.0000),
...
FROM governorates g, (VALUES
  ...
  ('2120', 'MoularÃƒÆ’Ã‚Â¨s', 34.3667, 8.4000),
  ('2130', 'Redeyef', 34.3833, 8.1333),
  ('2140', 'MÃƒÆ’Ã‚Â©tlaoui', 34.3167, 8.4000),
...
FROM governorates g, (VALUES
  ...
  ('4230', 'El GolÃƒÆ’Ã‚Â¢a', 33.4833, 9.0167),
...
SELECT g.id, code, place_name, latitude, longitude
FROM governorates g, (VALUES
  ('6000', 'GabÃƒÆ’Ã‚Â¨s', 33.8815, 10.0982),
  ('6011', 'GabÃƒÆ’Ã‚Â¨s Medina', 33.8815, 10.0982),
  ('6021', 'El Hamma', 33.8833, 9.8000),
  ('6031', 'Mareth', 33.6333, 10.2833),
  ('6041', 'MÃƒÆ’Ã‚Â©touia', 33.6333, 9.9167),
  ('6051', 'Nouvelle Matmata', 33.5333, 9.9667),
  ('6061', 'Menzel El Habib', 33.7833, 10.0667)
) AS data(code, place_name, latitude, longitude)
WHERE g.name = 'GabÃƒÆ’Ã‚Â¨s';
...
SELECT g.id, code, place_name, latitude, longitude
FROM governorates g, (VALUES
  ('4100', 'MÃƒÆ’Ã‚Â©denine', 33.3547, 10.5053),
...
('Signes GÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©raux', 1),
('Cardiovasculaire', 2),
('Dermatologique', 3),
('Digestif', 4),
('GÃƒÆ’Ã‚Â©nito-Urinaire', 5),
('Neuro-ostÃƒÆ’Ã‚Â©o-musculaire', 6),
('ORL', 7),
('Ophtalmologique', 8),
('Pleuro-Pulmonaire', 9),
('Psychiatrique', 10);

-- Comptes de base (admin / pharmacien / medecin)
-- Mot de passe par defaut pour les 3 comptes: password
INSERT INTO users (nom, prenom, email, password, role, isactivated)
VALUES
  ('Admin', 'Systeme', 'admin@vih.tn', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'admin', true),
  ('Pharmacien', 'Demo', 'pharmacien@vih.tn', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'pharmacien', true),
  ('Medecin', 'Demo', 'medecin@vih.tn', '$2b$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'medecin', true)
...
-- Stock Medicaments (reference l''admin cree ci-dessus)
WITH admin_user AS (
  SELECT id
  FROM users
...
INSERT INTO stock_medicaments (code, composition, quantite, dosage, created_by, updated_by)
