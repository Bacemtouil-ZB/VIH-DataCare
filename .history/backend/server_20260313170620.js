...
//  rate limiting
app.use("/api/auth", authLimiter); // strict sur auth
app.use("/api", globalLimiter); // général sur tout le reste
...
const startServer = async () => {
  try {
    ...
    app.listen(process.env.PORT, () => {
      console.log(`Server running sur http://localhost:${process.env.PORT}`);
    });
  } catch (err) {
    console.error("Impossible de se connecter à PostgreSQL :", err);
    process.exit(1);
