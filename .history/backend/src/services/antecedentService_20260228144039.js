...
// Section-by-section for a version (if you prefer no snapshot endpoint)
// Example: getVersionMedical(numero, versionNumber)...
export async function getVersionSection(numero, versionNumber, sectionId) {
  const client = await pool.connect();
  try {
    const patient = await requirePatient(client, numero);
    const antecedent = await requireAntecedentVersion(
    ...
    switch (sectionId) {
      case "medical":
        return { patient, antecedent, medical: await getMedical(client, antecedentId) };
      case "infectious":
        return { patient, antecedent, infectious: await getInfectious(client, antecedentId) };
      case "therapeutic":
        return { patient, antecedent, therapeutic: await getTherapeutic(client, antecedentId) };
      case "family":
        return { patient, antecedent, family: await getFamily(client, antecedentId) };
      case "gyneco":
        return { patient, antecedent, gyneco: await getGyneco(client, antecedentId) };
      case "surgical":
        return { patient, antecedent, surgical: await getSurgical(client, antecedentId) };
      case "transfusion":
        return { patient, antecedent, transfusion: await getTransfusion(client, antecedentId) };
      case "aes":
        return { patient, antecedent, aes: await getAes(client, antecedentId) };
      default: {
