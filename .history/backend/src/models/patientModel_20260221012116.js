...
export const updatePatient = async (id, patientData, updatedBy) => {
  ...
  try {
    ...
    // 1️⃣ Vérifie si les adresses doivent être mises à jour
    let birth_address_id = patientData.birth_address_id; // peut être null si non changé
    let residence_address_id = patientData.residence_address_id;

    // Si le medecin a changé le code postal naissance
    if (
      birth_postal_code_id &&
      birth_postal_code_id !== patientData.birth_postal_code_id
    ) {
      birth_address_id = await createAddress(
        client,
        birth_postal_code_id,
        null,
      );
    ...
    // Si le medecin a changé le code postal résidence
    if (
      residence_postal_code_id &&
      residence_postal_code_id !== patientData.residence_postal_code_id
    ) {
      residence_address_id = await createAddress(
        client,
        residence_postal_code_id,
        null,
      );
