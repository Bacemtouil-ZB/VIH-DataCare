...
export const updatePatient = async (id, patientData, updatedBy) => {
  ...
  try {
    ...
    // 1️⃣ Mise à jour des adresses si le code postal change
    if (
      birth_postal_code_id &&
      birth_postal_code_id !== patientData.currentBirthPostalCodeId
    ) {
    ...
    if (
      residence_postal_code_id &&
      residence_postal_code_id !== patientData.currentResidencePostalCodeId
    ) {
      await updateAddress(
        client,
        residence_address_id,
