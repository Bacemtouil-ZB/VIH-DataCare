...
export const updatePatient = async (id, patientData, updatedBy) => { const { numero, name, surname, birthdate, gender, birth_address_id, residence_address_id, phone, hospitalisation, last_visit_date, remarks, doctor_id, } = patientData; const query = UPDATE patients SET numero = COALESCE($1, numero), name = COALESCE($2, name), surname = COALESCE($3, surname), birthdate = COALESCE($4, birthdate), gender = COALESCE($5, gender), birth_address_id = COALESCE($6, birth_address_id), residence_address_id = COALESCE($7, residence_address_id), phone = COALESCE($8, phone), hospitalisation = COALESCE($9, hospitalisation), last_visit_date = COALESCE($10, last_visit_date), remarks = COALESCE($11, remarks), doctor_id = COALESCE($12, doctor_id), updated_by = $13, updated_at = NOW() WHERE id = $14 RETURNING *; ; const values = [ numero, name, surname, birthdate, gender, birth_address_id ?? null, residence_address_id ?? null, phone, hospitalisation, last_visit_date ?? null, remarks ?? null, doctor_id ?? null, updatedBy, id, ]; const result = await pool.query(query, values); if (result.rows.length === 0) { throw new Error("Patient non trouvé."); } return result.rows[0]; };
