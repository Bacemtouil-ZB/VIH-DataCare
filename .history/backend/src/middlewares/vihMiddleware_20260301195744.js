...
const validateDateLogic = (req, res, next) => {
  ...
  const dateStadeC =
    stade_cdc && STADES_C.includes(stade_cdc) && debut_stade_c ?
  ...
  if (dateNeg && datePositif && dateNeg >= datePositif) {
    return res.status(400).json({
      success: false,
      message:
        "La date du dernier test négatif doit être strictement antérieure à la date du test VIH positif",
    });
  }
  ...
  if (dateCont && datePositif && dateCont >= datePositif) {
    return res.status(400).json({
      success: false,
      message:
        "La date de contamination doit être strictement antérieure à la date du test VIH positif",
    });
  }
  ...
  if (dateStadeC && datePositif && dateStadeC < datePositif) {
    return res.status(400).json({
      success: false,
      message:
        "Le début du stade C ne peut pas survenir avant la date du test VIH positif",
    });
  }
...
export const validateCreateVih = [
  body("date_derniere_negative")
    ...
    .isDate()
    .withMessage(
      "La date du dernier test négatif doit être une date valide (YYYY-MM-DD)",
    )
  ...
  body("date_contamination")
    ...
    .isDate()
    .withMessage(
      "La date de contamination doit être une date valide (YYYY-MM-DD)",
    )
  ...
  body("date_vih_positif")
    ...
    .isDate()
    .withMessage(
      "La date du test VIH positif doit être une date valide (YYYY-MM-DD)",
    )
  ...
  body("debut_stade_c")
    ...
    .isDate()
    .withMessage(
      "La date de début du stade C doit être une date valide (YYYY-MM-DD)",
    )
