...
// ─── Helper format erreur unifié ──────────────────────────────────────────────
// Même format que handleValidation pour tous les middlewares custom

const sendFieldError = (res, field, message) =>
  res.status(400).json({
    success: false,
...
// Retourne errors[] avec field — même format que handleValidation ✅
...
export const validateDateLogic = (req, res, next) => {
  ...
  if (dateStadeC && datePositif && dateStadeC < datePositif) {
    return sendFieldError(
      res,
      "debut_stade_c",
      "Le début du stade C ne peut pas être avant la date VIH positif",
    );
  }
...
export const validateCreateVih = [
  ...
  handleValidation,
  validateDateLogic, // ← toujours après handleValidation
];
