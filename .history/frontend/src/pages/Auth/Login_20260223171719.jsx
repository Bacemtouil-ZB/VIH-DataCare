...
const Login = () => {
  ...
  // Nettoyer les erreurs champs au changement
  useEffect(() => {
    if (localError) setLocalError('');
    if (fieldErrors.email && formData.email) setFieldErrors(p => ({ ...p, email: '' }));
    if (fieldErrors.password && formData.password) setFieldErrors(p => ({ ...p, password: '' }));
  }, [formData.email, formData.password, fieldErrors.email, fieldErrors.password, localError]);
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <input
                ...
                value={formData.email}
                onChange={handleChange}
                className={`w-full px-3 py-2 bg-gray-100 text-black rounded-lg focus:outline-none focus:ring-2 placeholder-gray-400 border transition-colors ${
                  fieldErrors.email
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  value={formData.password}
                  onChange={handleChange}
                  className={`w-full px-3 py-2 pr-10 bg-gray-100 text-black rounded-lg focus:outline-none focus:ring-2 placeholder-gray-400 border transition-colors ${
            ...
            <button
              type="submit"
              disabled={isSubmitting || loading}
              className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg mt-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              {isSubmitting || loading ? (
                <>
                  <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
