      ...
      <header className="audit__header">
        <div>
          ...
          <p className="audit__subtitle">
            Recherche par numéro patient. Table scrollable + détails en modal.
          </p>
        </div>
      </header>
      ...
      <section className="audit__card">
        ...
        {selectedPatient && (
          <div className="audit__patient">
            <div className="audit__patientMain">
        ...
        <div className="audit__filters">
          <div className="audit__field">
            ...
            <input className="audit__input" value={module} onChange={(e) => { setOffset(0); setModule(e.target.value); }} />
          </div>
          <div className="audit__field">
            <label className="audit__label">Action</label>
            <input className="audit__input" value={action} onChange={(e) => { setOffset(0); setAction(e.target.value); }} />
          </div>
          <div className="audit__field">
            <label className="audit__label">Anomalie</label>
            <select className="audit__input" value={anomaly} onChange={(e) => { setOffset(0); setAnomaly(e.target.value); }}>
          ...
          <div className="audit__field">
            ...
            <input className="audit__input" type="date" value={from} onChange={(e) => { setOffset(0); setFrom(e.target.value); }} />
          </div>
          <div className="audit__field">
            <label className="audit__label">Au</label>
            <input className="audit__input" type="date" value={to} onChange={(e) => { setOffset(0); setTo(e.target.value); }} />
          </div>
        </div>
      </section>
      ...
      <section className="audit__card audit__card--table">
        <div className="auditTable">
          <table>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={9} className="audit__cell audit__muted">Chargement…</td></tr>
              ) : logs.length === 0 ? (
                <tr><td colSpan={9} className="audit__cell audit__muted">Aucun log. Recherchez un patient.</td></tr>
              ) : (
                logs.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="audit__cell">
                      <span className={`auditTag ${l.is_anomaly ? "auditTag--danger" : "auditTag--ok"}`}>
        ...
        {/* Pagination always visible (sticky) */}
            ...
            <div className="auditModal__body">
              {detailsLoading ? (
                ...
                <>
                  ...
                  <div className="auditModal__jsonActions">
                    <button className="audit__btn audit__btn--sm" type="button" onClick={() => navigator.clipboard.writeText(JSON.stringify(details.old_data ?? {}, null, 2)).then(() => toast.success("old_data copié")).catch(() => toast.error("Impossible de copier"))}>
                      <i className="bi bi-clipboard" /> Copier old_data
                    </button>
                    <button className="audit__btn audit__btn--sm" type="button" onClick={() => navigator.clipboard.writeText(JSON.stringify(details.new_data ?? {}, null, 2)).then(() => toast.success("new_data copié")).catch(() => toast.error("Impossible de copier"))}>
                      <i className="bi bi-clipboard" /> Copier new_data
                    </button>
                  </div>
