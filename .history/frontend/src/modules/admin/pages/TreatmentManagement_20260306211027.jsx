    ...
    <div className="auditPage">
      <div className="auditContainer">
        <div className="auditHeader">
          ...
          <p className="auditSubtitle">Rechercher un patient par numéro et consulter la traçabilité.</p>
        </div>
            ...
            <div className="auditPatientBanner">
              <div>
                <div className="auditPatientName">
                  Patient: {selectedPatient.numero} — {selectedPatient.name} {selectedPatient.surname}
                </div>
        ...
        <div className="auditCard">
          <div className="auditTableWrap">
            <table className="auditTable">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={9} className="auditTd">Chargement...</td>
                  </tr>
                ) : logs.length === 0 ? (
                  <tr>
                  ...
                  logs.map((l) => (
                    <tr key={l.id}>
                      ...
                      <td className="auditTd">
                        <span className={`auditBadge ${l.is_anomaly ? "isYes" : "isNo"}`}>
          ...
          <div className="auditFooter">
            ...
            <div className="auditFooterRight">
              <button className="auditBtn auditBtnGhost" type="button" onClick={prevPage} disabled={page <= 1}>
                <FiChevronLeft /> Précédent
              </button>
              <button className="auditBtn auditBtnGhost" type="button" onClick={nextPage} disabled={page >= totalPages}>
        ...
        {/* Details Modal */}
        {detailsOpen && (
          <div className="auditModalOverlay" onClick={() => setDetailsOpen(false)}>
            <div className="auditModal" onClick={(e) => e.stopPropagation()}>
              <div className="auditModalHeader">
                ...
                <button className="auditBtn auditBtnGhost" type="button" onClick={() => setDetailsOpen(false)}>
                  <FiX /> Fermer
                </button>
              </div>
                  ...
                  <>
                    <div className="auditMetaGrid">
                      <div className="auditMetaBox">
                        ...
                        <div className="auditMetaVal">{formatDateTime(selectedLogDetails.created_at)}</div>
                      </div>

                      <div className="auditMetaBox">
                        <div className="auditMetaKey">Utilisateur</div>
                        <div className="auditMetaVal">
                          {safeText(selectedLogDetails.user_nom)} {safeText(selectedLogDetails.user_prenom)}
                        </div>
                        <div className="auditMuted">{safeText(selectedLogDetails.user_email)}</div>
                      </div>

                      <div className="auditMetaBox">
                        <div className="auditMetaKey">Réseau</div>
                        <div className="auditMetaVal">IP: {safeText(selectedLogDetails.ip_address)}</div>
                        <div className="auditMuted">UA: {safeText(selectedLogDetails.user_agent)}</div>
                        <div className="auditMuted">Request: {safeText(selectedLogDetails.request_id)}</div>
                      </div>
                    </div>
                    ...
                    <div className="auditJsonGrid">
                      <div>
                        <div className="auditJsonTitle">Anciennes données (old_data)</div>
                        <pre className="auditPre">
                          {JSON.stringify(selectedLogDetails.old_data, null, 2)}
                        </pre>
                      </div>
                      <div>
                        <div className="auditJsonTitle">Nouvelles données (new_data)</div>
                        <pre className="auditPre">
                          {JSON.stringify(selectedLogDetails.new_data, null, 2)}
                        </pre>
                      </div>
