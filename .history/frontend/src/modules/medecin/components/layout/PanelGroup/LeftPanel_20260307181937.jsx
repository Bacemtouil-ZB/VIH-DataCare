...
export default function LeftPanel() {
  ...
  // Watch panel width with ResizeObserver
  useEffect(() => {
  ...
  const isNarrow = panelWidth < 150;   // hide text, show avatar only
  const isTiny   = panelWidth < 60;    // fully hidden — show nothing

  useEffect(() => {
    if (!numero) { setError("Aucun numéro de dossier fourni"); setLoading(false); return; }
    if (numero === "new") { setPatientData(null); setDerniersPrises([]); setError(null); setLoading(false); return; }
  ...
  // ── Render ──────────────────────────────────────────────────────────
  if (isTiny) return <div ref={panelRef} className="patient-panel patient-panel--tiny" />;

  if (loading) return (
    <div ref={panelRef} className="patient-panel">
      <div className="loading-container">
        <div className="spinner" />
        {!isNarrow && <p>Chargement...</p>}
      </div>
    </div>
  );
  ...
  // NARROW MODE — avatar only
  if (isNarrow) return (
    <div ref={panelRef} className="patient-panel patient-panel--narrow">
      <div className="patient-avatar patient-avatar--sm">
        {patientPhotoUrl
          ? <img src={patientPhotoUrl} alt="Patient" className="patient-photo patient-photo--sm" />
