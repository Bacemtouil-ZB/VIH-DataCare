...
<div className="form-group">
  ...
  <div className="form-group"> 
    ...
    <select name="birth_address_id" value={formData.birth_address_id || ""} 
        onChange={onChange} disabled={!isEditing} > 
        <option value="">Sélectionner</option> 
          {addresses.map((addr) => ( <option key={addr.id} 
          value={addr.id}> {addr.governorate} - {addr.code_postal}
        </option> ))}
    </select> 
  </div>
