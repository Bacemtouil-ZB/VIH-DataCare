...
export default function ProfilForm({
  ...
  // Gouvernorats uniques depuis addresses
  const governorates = [...new Set(addresses.map(a => a.governorate))];

  // Filtrer codes postaux selon gouvernorat choisi
        ...
        <fieldset className="form-section">
          ...
          <div className="form-group">
            ...
            >
              <option value="">Sélectionner</option>
              {doctors.map(doc => (
                <option key={doc.id} value={doc.id}>{doc.nom} {doc.prenom}</option>
              ))}
            </select>
          </div>
        </fieldset>
