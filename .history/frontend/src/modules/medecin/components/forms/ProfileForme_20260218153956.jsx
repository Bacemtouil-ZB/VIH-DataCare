...
export default function ProfilForm({
  ...
  /* ------------------ LISTES ------------------ */
  // Gouvernorats uniques avec id
  const uniqueGovernorates = [
    ...new Map(addresses.map(a => [a.governorate_id, a.governorate])).entries()
  ].map(([id, name]) => ({ id, name }));

  // Codes postaux filtrés par gouvernorat
  const filteredBirthPostalCodes = addresses.filter(
    a => a.governorate_id === parseInt(formData.birth_governorate)
  );
  const filteredResidencePostalCodes = addresses.filter(
    a => a.governorate_id === parseInt(formData.residence_governorate)
  );
