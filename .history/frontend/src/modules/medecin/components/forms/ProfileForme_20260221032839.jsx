       ...
       {/* Numéro dossier */}
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => {
              let value = e.target.value.replace(/[<>]/g, ""); // protection contre < > 

              // Validation immédiate
              const currentYear = new Date().getFullYear();
              const regex = /^(F\s)?(\d{3})\/(\d{4})$/;
              const match = value.match(regex);

              if (match) {
                const year = parseInt(match[3], 10);
                if (year > currentYear) {
                  toast.error(`L'année doit être inférieure ou égale à ${currentYear}`);
                } else {
            ...
            disabled={!isNew}
            required
            placeholder="Ex: F 001/2025 ou 001/2025"
          />
          {/* Message d'erreur */}
          {formData.numero && !/^(F\s)?\d{3}\/\d{4}$/.test(formData.numero) && (
            <span className="error-text">
              Format invalide : ex. F 001/2025 pour externe ou 001/2025 pour interne
            </span>
          )}
        </div>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            setFormData(prev => {
              ...
              // Ajouter F automatiquement si externe
              if (hospitalisation === "externe" && !numero.startsWith("F ")) {
                numero = `F ${numero}`;
              }
              // Supprimer F si interne
              if (hospitalisation === "interne") {
                numero = numero.replace(/^F\s*/, "");
              }
