  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        // Charger adresses
        const addr = await getAllAddresses();
        setAddresses(addr || []);

        // Charger patient si existant
        if (!isNew) {
          const data = await getPatientByNumero(numero);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isNew) {
      const result = await Alert.fire({
        ...
        html: `
          <div class="text-start">
            <p class="mb-2">Les changements seront appliqués au dossier patient.</p>
          </div>
        `,
        icon: "question",
        showCancelButton: true,
