...
export function useProfileLogic() {
  ...
  //   const raw = e.target.value.replace(/[^0-9]/g, ""); // chiffres seulement
  //   const part2 = raw.slice(4, 8);

  //   // ── détecte année future AVANT normalisation ──
  //   if (part2.length === 4) {
  ...
  const handleNumeroChange = (e) => {
    ...
    // Ajouter le tiret après le 4ᵉ chiffre immédiatement
    let value;
    if (raw.length > 4) {
      value = raw.slice(0, 4) + "-" + raw.slice(4, 8);
    } else if (raw.length === 4) {
      value = raw + "-"; // exactement 4 chiffres → ajoute le tiret
    } else {
      value = raw;
    }
