  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (numero === "new") {
        ...
        const numeroSaisi = formData.numero;
        const exists = await getPatientByNumero(numeroSaisi); // lightweight API call
        if (exists) {
          setErrors({ numero: "Ce numéro patient existe déjà." });
          return; // stop here, don't call createPatient
        }
