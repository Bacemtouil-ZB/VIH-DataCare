...
export function useProfileLogic() {
  ...
  //   const error = validateNumeroYear(value); // ← reçoit le message
  //   if (error) toast.error(error); // ← toast ici dans le hook
  ...
  const handleNumeroChange = (e) => {
    ...
    // si utilisateur tape "-" → toast info + bloquer
    if (
      raw.includes("-") &&
      !e.target.value.replace(/^F-/, "").match(/^\d+-/)
    ) {
      toast.info("Le tiret est ajouté automatiquement après 3 ou 4 chiffres");
    }

    const value = normalizeNumero(raw);
    const error = validateNumeroYear(value);
