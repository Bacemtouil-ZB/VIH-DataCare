...
export default function AntecedentsForm() {
  ...
  const [, force] = useState(0); // small rerender trigger for dirty badge

  const canEdit = !readOnly && isEditing;

  const ensureActiveExists = useCallback(async () => {
    const header = await antecedentsService.getActiveAntecedent(numero);
    if (!header?.antecedent) await antecedentsService.postNewAntecedentVersion(numero);
  }, [numero]);
  ...
  const loadSnapshot = useCallback(async (versionNumber) => {
    if (!numero || !versionNumber) return;
    setLoading(true);
    try {
  ...
  const setDirty = (sectionId, nextValue) => {
    const saved = savedRef.current?.[sectionId];
    const dirty = !isSame(saved, nextValue);

    if (dirty) dirtyRef.current.add(sectionId);
    else dirtyRef.current.delete(sectionId);

    force((x) => x + 1);
  };

  // one helper covers object + list updates
  const patchSection = (sectionId, producer) => {
    if (!canEdit) return;
    setForm((f) => {
      const current = f[sectionId];
      const nextSection = producer(current);
      const next = { ...f, [sectionId]: nextSection };
      setDirty(sectionId, nextSection);
      return next;
    });
  };
  ...
  const removeRow = (sectionId, index) =>
    patchSection(sectionId, (cur) => (Array.isArray(cur) ? cur.filter((_, i) => i !== index) : []));
  ...
  const handleSaveAll = async () => {
    ...
    if (dirtyCount === 0) return toast.info("Aucune modification à enregistrer");
      ...
      <div className="antecedents-header">
        <div className="header-content">
          <div className="header-left" style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <button onClick={handleNewVersion} className="btn-secondary" disabled={loading || saving}>
              + Nouvelle version
            </button>
          ...
          <div className="header-right" style={{ display: "flex", gap: 10 }}>
            {!isEditing ? (
              ...
              <>
                <button onClick={handleSaveAll} className="btn-primary" disabled={saving || loading || readOnly}>
                  {saving ? "Enregistrement..." : "Enregistrer"}
                </button>
