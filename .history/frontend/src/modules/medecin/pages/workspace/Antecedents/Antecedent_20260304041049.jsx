...
// shallow compare (enough for our usage because we always replace section objects/arrays)
...
export default function AntecedentsForm() {
  ...
  // ✅ edit mode like profile/social
  const [isEditing, setIsEditing] = useState(false);

  // ✅ snapshot used for Cancel and for dirty checking
  const savedFormRef = useRef(initialState);
  ...
  const applySnapshotToForm = useCallback((snapshot) => {
    ...
    for (const section of SECTIONS) {
      const id = section.id;
      const normalized = normalizeFromApi(id, snapshot?.[id]);
      next[id] = normalized ?? (Array.isArray(initialState[id]) ? [] : { ...initialState[id] });
    }

    setForm(next);

    // ✅ update saved snapshot (cancel target)
    savedFormRef.current = next;

    // ✅ nothing dirty after loading snapshot
  ...
  const loadSnapshot = useCallback(
    async (versionNumber) => {
      ...
      try {
        const snap = await antecedentsService.getAntecedentVersionSnapshot(numero, versionNumber);
  ...
  // ✅ Update helpers: mark section dirty
  const markDirty = (sectionId, nextSectionValue) => {
    const saved = savedFormRef.current?.[sectionId];
    const dirty = !isSame(saved, nextSectionValue);
  ...
  const updateSection = (sectionId, key, value) => {
    if (!canEdit) return;
    setForm((f) => {
      const nextSection = { ...(f[sectionId] || {}), [key]: value };
      const next = { ...f, [sectionId]: nextSection };
      markDirty(sectionId, nextSection);
      return next;
    });
  };
  ...
  const updateList = (sectionId, index, key, value) => {
    if (!canEdit) return;
    setForm((f) => {
      const arr = Array.isArray(f[sectionId]) ? [...f[sectionId]] : [];
      arr[index] = { ...(arr[index] || {}), [key]: value };
      const next = { ...f, [sectionId]: arr };
      markDirty(sectionId, arr);
      return next;
    });
  };
  ...
  const addRow = (sectionId, template) => {
    ...
    setForm((f) => {
      const arr = Array.isArray(f[sectionId]) ? [...f[sectionId]] : [];
      const nextArr = [...arr, { ...template }];
      const next = { ...f, [sectionId]: nextArr };
      markDirty(sectionId, nextArr);
      return next;
    });
  };
  ...
  // ✅ Like Profile: confirm before editing
  ...
  const handleSaveAll = async () => {
    ...
    if (!isEditing) {
      toast.info("Cliquez sur “Modifier” avant d’enregistrer");
      return;
    }
    ...
    const ok = await confirmAction({
      ...
      text: `Vous allez enregistrer ${dirtySections.size} section(s).`,
      confirmButtonText: "Enregistrer",
    });
    if (!ok) return;
    ...
    try {
      ...
      for (const sectionId of Array.from(dirtySections)) {
        const api = apiBySection[sectionId];
        if (!api?.put) continue;

        const payload = sanitizeForApi(sectionId, form[sectionId]);
        await api.put(numero, payload);
      }

      toast.success("Toutes les sections modifiées ont été enregistrées");

      // reload snapshot from backend to ensure consistency
      await loadVersions();
      await loadSnapshot(selectedVersion);
      ...
      <div className="antecedents-header">
        <div className="header-content">
          <div className="header-left" style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            {readOnly ? (
              <span style={{ fontSize: 12.5, color: "#92400e", background: "#fffbeb", border: "1px solid #f59e0b", padding: "4px 8px", borderRadius: 999 }}>
                Lecture seule (archivée)
              </span>
            ) : null}

            {!readOnly && dirtySections.size > 0 ? (
              <span style={{ fontSize: 12.5, color: "#1f2937", background: "#eef2ff", border: "1px solid #c7d2fe", padding: "4px 8px", borderRadius: 999 }}>
                {dirtySections.size} section(s) modifiée(s)
              </span>
            ) : null}
          </div>
          ...
          <div className="header-right" style={{ display: "flex", gap: 10 }}>
            {!isEditing ? (
              <button onClick={handleEdit} className="btn-secondary" disabled={loading || saving || readOnly}>
                Modifier
              </button>
            ) : (
              <>
                ...
                <button onClick={handleCancel} className="btn-secondary" disabled={saving || loading}>
                  Annuler
                </button>
              </>
            )}
      ...
      <div className="antecedents-container">
        ...
        {loading ? (
          ...
          <SectionRenderer
            ...
            addRow={addRow}
            removeRow={removeRow}
            readOnly={!canEdit} // ✅ locked unless editing
          />
        )}
      </div>
    </div>
