...
export function useSignesCliniquesLogic(numero, examenId) {
  ...
  const handleSave = async () => {
    // ── Validation frontend → FieldError sous chaque champ ───
    const frontendErrors = {};
    if (!taille) frontendErrors.taille = "Taille est requise";
    else if (+taille <= 0 || +taille > 250)
      frontendErrors.taille = "Taille invalide (1-250 cm)";
    if (!poids) frontendErrors.poids = "Poids est requis";
    else if (+poids <= 0 || +poids > 300)
      frontendErrors.poids = "Poids invalide (1-300 kg)";
