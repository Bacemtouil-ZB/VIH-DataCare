...
export function useSignesCliniquesLogic(numero, examenId) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Si le backend renvoie des erreurs de validation (status 400)
      if (err?.errors && Array.isArray(err.errors)) {
        const formattedErrors = {};
        err.errors.forEach((e) => {
          formattedErrors[e.field] = e.message; // 'field' doit correspondre au nom du champ
        });
        setErrors(formattedErrors);
        return; // Stop ici pour éviter le toast global
      }
